...
class TheoryTestCertificate extends \TheoryTest\Car\TheoryTestCertificate{
    ...
    public function generateCertificate(){
        ...
        if($this->theory->testresults['status'] == 'pass'){
            ...
            $this->pdf->Ln(12);
            $this->certLine('Name:', self::$user->getFirstname($this->theory->getUserID()).' '.self::$user->getLastname($this->theory->getUserID()));
            $this->pdf->Ln(10);
            $this->pdf->SetFont('Arial','B', 18);
        ...
        $details = array(array(self::$user->getFirstname($this->theory->getUserID()).' '.self::$user->getLastname($this->theory->getUserID()), strip_tags($this->theory->getTestName()), $this->theory->testresults['id'], date('d/m/Y g:i A', strtotime($this->theory->testresults['complete']))));
        ...
        if($this->theory->testresults['status'] == 'pass'){
            $this->pdf->Cell(184, 10, "Congratulations ".self::$user->getFirstname($this->theory->getUserID())); $this->pdf->Ln(4);
            ...
            $this->pdf->Cell(184, 10, "Sorry ".self::$user->getFirstname($this->theory->getUserID()).", but you have not passed this time."); $this->pdf->Ln(4);
        ...
        $this->pdf->Ln(6);
        if($this->testType != 'free'){$this->infoLine('Candidate:', self::$user->getFirstname($this->theory->getUserID()).' '.self::$user->getLastname($this->theory->getUserID()));}
